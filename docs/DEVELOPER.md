...
| Параметр | По умолчанию | Описание |
|----------|--------------|----------|
| `api_key` | `None` | Статичный секрет; если задан — все эндпоинты API (кроме `GET /health`) требуют ключ в заголовке `Authorization: Bearer <key>` |
...
Разработка ведётся итерациями с тегами SemVer (с v1.0.0 — стабильные релизы).
...
### v0.7.0 – Аутентификация по API-ключу
- параметр `api_key` в конструкторе `Broker` (статичный секрет)
- middleware или dependency: проверка ключа в заголовке запроса на всех эндпоинтах API
- без валидного ключа — `401 Unauthorized`; посторонние не могут публиковать, тянуть и управлять задачами
- `GET /health` остаётся без ключа (liveness для оркестраторов)
- тесты: отсутствие ключа, неверный ключ, успешный доступ

### v1.0.0 – Стабильный релиз
- SemVer 1.0.0: зафиксированный публичный API (publish, pull, lifecycle, list, metrics, auth)
- PostgreSQL, CI (GitHub Actions), примеры запросов в `docs/API_EXAMPLES.md`
...
- инкремент метрик при publish, pull, ack, nack
